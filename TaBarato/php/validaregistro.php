<?php
    ...
    if($cnpj_formatado){
        ...
        if(mysqli_num_rows($resultado) == 0){ // Verifica se esse Email já foi cadastrado.
            $query = "SELECT * 
                      FROM mercados 
                      WHERE CNPJ = '$cnpj_formatado'";
            $resultado = mysqli_query($conexao, $query);
            if(mysqli_num_rows($resultado) == 0){ // Verifica se esse CNPJ já foi cadastrado.
                ...
                if($resultado){
                    ...
                    header("location: ../criar_conta/cadastro.php"); // Editar página //
                }
                else{
    ...
    else{ // Se a função retornar falso.
